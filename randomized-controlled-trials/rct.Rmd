...
denominator <- sum(keller_df$keller_trt_yes) / sum(keller_df$keller_trt)

att <- numerator / denominator 
print(round(att, 2))
```

This approximately gives us the ATT, and since we assume there was no selection bias in treatment assignment, ATE as well. Therefore, we can conclude by saying that the average treatment effect of disconnection is a reduction in electricity payments by 8,593.11 rupees. Noncompliance appears to have led to an underestimation of the ATE by around 2,550 rupees. 